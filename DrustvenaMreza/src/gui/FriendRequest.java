...
//	}
	 ...
	 * Create the dialog.
	 */
	public FriendRequest(Korisnik k) {
		user = k;
		...
		List<Korisnik> users = Crud.getRequests(user);
		...
		});
		{
			...
			{
				JButton btnAdd = new JButton("Accept friend");
				btnAdd.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						...
						boolean friendship = Crud.acceptFriend(user, to);
						if(friendship) {
							JOptionPane.showMessageDialog(contentPanel, "Just accepted a friend with username: " + username);
							if(maxSlIndex != -1) {
